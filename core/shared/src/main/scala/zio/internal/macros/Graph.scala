  ...
  def buildComplete(outputs: List[Key]): Either[::[GraphError[Key, A]], LayerTree[A]] = {
    ...
    if (!outputs.isEmpty) 
      for {
        ...
        leftTree <- if(dependencies.isEmpty) Right(LayerTree.empty) else Right(throw new Throwable(List(d1.toString, n1.toString, dependencies.toString, neededKeys.toString).mkString("n")))
