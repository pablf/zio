...
object GraphError {
  def missingTransitiveDependency[Key, A](node: Option[Node[Key, A]], dependency: Key): GraphError[Key, A] =
    MissingTransitiveDependencies(node, Chunk(dependency))

  case class MissingTransitiveDependencies[+Key, +A](node: Option[Node[Key, A]], dependency: Chunk[Key])
